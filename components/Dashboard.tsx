...
const Dashboard: React.FC<Props> = ({ data, onUpdateLog, onDeleteLog, onImport, onReset, onDemoLoad, lang, setLang }) => {
      ...
      <header className="bg-white shadow-sm sticky top-0 z-10 transition-all duration-300">
        <div className="max-w-md mx-auto px-4 py-3 flex justify-between items-center">
          <h1 className="text-lg font-bold text-emerald-700 flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-emerald-500" /> {tApp.appTitle}
          </h1>

          <div className="flex bg-gray-100 p-1 rounded-full border border-gray-200">
            <button
              onClick={() => setLang('pt')}
              className={`w-8 h-8 flex items-center justify-center rounded-full text-lg transition-all ${lang === 'pt' ? 'bg-white shadow-sm scale-105' : 'opacity-40 grayscale hover:opacity-70'}`}
            >
              🇧🇷
            </button>
            <button
              onClick={() => setLang('en')}
              className={`w-8 h-8 flex items-center justify-center rounded-full text-lg transition-all ${lang === 'en' ? 'bg-white shadow-sm scale-105' : 'opacity-40 grayscale hover:opacity-70'}`}
            >
              🇺🇸
            </button>
          </div>
              ...
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs font-medium rounded-full transition-colors ${timeRange === range.value
